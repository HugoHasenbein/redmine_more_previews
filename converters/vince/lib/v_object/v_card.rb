...
module VObject
  class VCard
    ...
    ######################################################################################
    def write
      VObject::Writer.new(:object => self).write
    end #def
    
    def humanize(fields:nil)
      VObject::Writer.new(fields:fields,:object => self).humanize
    end #def
    
    def webalize(fields:nil,iconize: false)
      VObject::Writer.new(:object => self).webalize(fields:fields,iconize: iconize)
    end #def
    ...
    PHOTO_WEBALIZE_PROC = Proc.new do |hash|
      ...
      when /file/i
        ...
        mime = nil; File.open( filepath ) {|f| mime = MimeMagic.by_magic(f).try(:type)}
