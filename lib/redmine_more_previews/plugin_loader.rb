...
module RedmineMorePreviews
  class PluginPath
    attr_reader :assets_dir, :initializer, :views, :public_directory
    ...
    def initialize(dir)
      ...
      @public_directory = PluginLoader.public_directory
    end
  ...
  class PluginLoader
    ...
    def self.setup
      ...
      Dir.glob(File.join(directory, '*')).sort.each do |directory|
        ...
        @plugin_directories << RedmineMorePreviews::PluginPath.new(directory)
      end
    end
...
#         engine_cfg.paths['lib'].eager_load! # create all paths
          ...
          ActiveSupport::Dependencies.autoload_paths   += (engine_cfg.eager_load_paths + 
                                                           ...
                                                           [File.join(PluginLoader.public_directory, 'lib')]) # lib was already added in self.load
        else # > = Redmine 5
